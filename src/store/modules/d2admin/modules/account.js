...
export default {
  ...
  actions: {
     ...
     * @param {Object} payload userPhone {String} 用户账号
    ...
    async login ({ dispatch }, {
      ...
      // const res = await api.SYS_USER_LOGIN({ userPhone, userPassword })
      let that = window.vue;
      that.axios.post('/public/login', {userPhone:userPhone,userPassword:userPassword})
        .then(function(r) {
          if(r.code === 0) {
            if (isPwd) {
              that.hk.data('remember',{
                key:userPhone,
                value : {userPhone:userPhone,userPassword:userPassword,isPwd:isPwd}
              });
            } else {
              that.hk.data('remember',{
                key:userPhone,
            ...
            util.cookies.set('uuid', userPhone)
            util.cookies.set('token', userPhone+"1")
            // 设置 vuex 用户信息
            dispatch('d2admin/user/set', { name: userPhone+"2" }, { root: true })
            // 用户登录后从持久化数据加载一系列的设置
            dispatch('load')
          }
          if (r.code === 1) {
